...
interface ComponentsInput {
    buttons?: Array<{ customId: string; items: { item: unknown[] } }>;
    select?: Array<{ customId: string; selectType: string; maxValues?: number; placeholder?: string; options: { option: unknown[] } }>;
    textEdit?: Array<{ customId: string; title: string; saveLabel?: string; placeholder?: string; maxLength?: number }>;
    fileUpload?: Array<{ customId: string; title?: string; buttonLabel?: string; multiple?: boolean; minFiles?: number; maxFiles?: number; maxSizeMb?: number }>;
    richChoices2?: Array<{ items: { item: unknown[] } }>;
    imageChoices?: Array<{ customId: string; items: { item: unknown[] } }>;
    checkboxes?: Array<{ customId: string; options: { option: unknown[] } }>;
    note?: Array<{ text: string }>;
    progress?: Array<{ text?: string }>;
    image?: Array<{ imageUrl: string; label?: string }>;
}
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                const components = this.getNodeParameter('components', i) as ComponentsInput;
                const metadata = this.getNodeParameter('metadata', i, {}) as MetadataInput;
                ...
                if (components.buttons && components.buttons.length > 0) {
                    for (const buttonGroup of components.buttons) {
                        ...
                        if (buttonGroup.items && buttonGroup.items.item) {
                            for (const btn of buttonGroup.items.item as Array<Record<string, unknown>>) {
                ...
                if (components.richChoices2 && components.richChoices2.length > 0) {
                    for (const richChoice of components.richChoices2) {
                        ...
                        if (richChoice.items && richChoice.items.item) {
                            for (const item of richChoice.items.item as Array<Record<string, unknown>>) {
                ...
                if (components.imageChoices && components.imageChoices.length > 0) {
                    for (const imageChoice of components.imageChoices) {
                        ...
                        if (imageChoice.items && imageChoice.items.item) {
                            for (const item of imageChoice.items.item as Array<Record<string, unknown>>) {
                                items.push({
                ...
                if (components.checkboxes && components.checkboxes.length > 0) {
                    for (const checkbox of components.checkboxes) {
                        ...
                        if (checkbox.options && checkbox.options.option) {
                            for (const opt of checkbox.options.option as Array<Record<string, unknown>>) {
                                options.push({
                                    label: opt.label,
                                    value: opt.value,
                                });
